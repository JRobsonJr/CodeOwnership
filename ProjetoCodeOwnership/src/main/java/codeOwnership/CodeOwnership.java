...
public class CodeOwnership {
	...
	public void creatPairs(Repository repo) throws Exception {
		...
		for (RevCommit commit : commits) {
			...
			try {
				...
				System.out.println(pairs.toString()); // erro do primeiro commit
			}

			for (DiffEntry entry : diffFormatter.scan(diffWith, commit)) {
				if (isNewArtifact(entry) && isJavaClass(entry.getNewPath())) {

					PairStudentClass auxPair = new PairStudentClass(
							students.getStudent(commit.getAuthorIdent().getEmailAddress()), entry.getNewPath());
